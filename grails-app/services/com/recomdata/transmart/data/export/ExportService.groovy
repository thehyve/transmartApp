...
class ExportService {
    ...
    def getsubsetSelectedPlatformsByFiles(checkboxList) {
        ...
        checkboxList.each {checkboxItem ->
            def checkboxItemMatcher = checkboxItem =~ /^([^_]+)_([^_]+)_([^_]+)_(.+)$/

            if (checkboxItemMatcher) {
                def currentSubset = checkboxItemMatcher[0][1].trim()
                def fileName = checkboxItemMatcher[0][2].trim() + checkboxItemMatcher[0][3].trim()
                def platform = checkboxItemMatcher[0][4].trim()
    ...
    def private createExportDataJob(userName, params, statusList) {
        ...
        if (checkboxList instanceof String) {
            def tempArray = []
            if (checkboxList && !checkboxList?.trim().equals("")) {
                tempArray.add(checkboxList)
            }
