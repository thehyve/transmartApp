...
class RWGController {
    ...
    */
     ...
     */
    private def executeSOLRFacetedQuery(solrRequestUrl, solrQueryParams, returnAnalysisIds = false) {
        ...
        def facetCategoryNodes
        // will store the facet category nodes from the xml response in here
        ...
        for (catNode in facetCategoryNodes) {
            ...
            // json object for current category
            for (countNode in catNode.int) {
                def skId = countNode.@name
                // search keyword id
    ...
    def getFacetResultsForTable = {
        ...
        try {
            nonfacetedQueryString = trialQueryService.createSOLRNonfacetedQueryString(sessionFilterParams as List)
        }
        catch (Exception e) {
            e.printStackTrace()
        }
    ...
    private String setSOLRGenesField(boolean showSigGenesOnly = true) {
        ...
        def solrGenesField = ""
        // name of SOLR search field to be used for gene queries (SIGGENE or ALLGENE)
        if (showSigGenesOnly) {
    ...
    def loadSearchResults = {studyCounts, startTime ->
        def trialAnalysis = [:]
        // Map of the trial objects and the number of analysis per trial
        def total = 0
        // Running total of analysis to show in the top banner
    ...
    JSONObject getFacetedSearchKeywordsAndAnalyses(long id) {
        ...
        if (s == null) {
            ...
            ids.each {
                JSONObject termArray = new JSONObject()
                // json object for current search term
                def skId = it
                // search keyword id
            ...
            if (analysesPiped) {
                ...
                def results = org.transmart.biomar.BioAssayAnalysis.executeQuery("select b.id, b.longDescription " +
                        " from org.transmart.biomart.BioAssayAnalysis b" +
                        " where b.id in (" + aIds.join(',') + ") ")

                aIds.each {aId ->
                    JSONObject analysisArray = new JSONObject()
                    // json object for current analyses
                    def found = false
                    for (def r = 0; r < results.size(); r++) {
                        ...
                        if (row[0].toString() == aId.toString()) {
                            ...
                            def studyInfo = org.transmart.biomar.BioAnalysisAttribute.executeQuery("select distinct studyID from BioAnalysisAttribute where bioAssayAnalysisID=?", [row[0]])
    ...
    def getHomePage = {
        ...
        def ta = SearchTaxonomy.findByTermName('Therapeutic Areas')
        //default to ta
        def d = SearchTaxonomy.findByTermName('Disease')
        //default to disease
