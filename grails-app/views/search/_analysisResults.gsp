	...
	<div id="analysis_results_table_${analysisId}_length_wrapper" class="dataTables_length">
		<label>Show 
			<g:select name="rows_per_page" size="1" from="${['10':'10','25':'25','50':'50','100':'100']}" optionKey="${{it.key}}" optionValue="${{it.value}}" id="analysis_results_table_${analysisId}_length" onchange="loadAnalysisResultsGrid(${analysisId}, {max: jQuery(this).val()})" value="${max}"/> entries
		</label>	
	</div>
				...
				<g:each in="${analysisData}" var="row" status="i">
					...
					<g:each in="${row}" var="data" status="colNum">
						<g:unless test="${colNum == 0}"> <%-- Skip analysis name --%>
							<td><g:typeIndifferentNumberRound number="${data}"></g:typeIndifferentNumberRound></td>
						</g:unless>
					</g:each>
					</tr>
				</g:each>
