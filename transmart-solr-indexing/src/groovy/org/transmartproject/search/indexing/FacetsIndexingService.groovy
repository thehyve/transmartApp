...
class FacetsIndexingService implements InitializingBean {
    ...
    void indexByTypes(Set<String> types /* null for all */) {
        List<Map.Entry<FacetsIndexingModule, FacetsDocId>> entries
        Multimap<FacetsDocId, FacetsIndexingModule> ids = HashMultimap.create()
        modules.each { FacetsIndexingModule mod ->
            def relevantTypes = mod.supportedDocumentTypes
            if (types) {
                relevantTypes = Sets.intersection(relevantTypes, types)
            }
            relevantTypes.each { type ->
